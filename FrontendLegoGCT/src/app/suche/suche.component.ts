...
export class SucheComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    // @ts-ignore
    let verlauf_id: string = localStorage.getItem("verlaufSuche");
    if (verlauf_id != undefined) {
      this.clickSuggestion(verlauf_id);
      localStorage.removeItem("verlaufSuche");
  ...
  gibGCTLogo():string {
   if(this.istDarkmode()) {
     return "./assets/GCT-logos_yellow_neu.png"
   }else {
     return "./assets/GCT-logos_black.png"

   }
  }

  /**
   * methode zum Prüfen ob der Dark-mode aktiv ist
   */
  istDarkmode(): boolean {
    // @ts-ignore
   ...
   */
  getVorschlaege() {
    //toPromis sichert zu, dass alle daten empfangen wurden
  ...
  /**
   * gibt eine Liste mit Vorschlägen für die Sucheingabe
   */
  async getVorschlaegListe() {

    if (this.eingabeWert.length > 2 && /[A-Za-z]/.test(this.eingabeWert)) {
      //öffnet das Such-overlay
      this.isOpen = true;
      let vorschlaege_liste: VorschlagElement[] = [];
      //vollständiges laden der Vorschläge
